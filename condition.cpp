...
bool Condition::estResultatAutreRegle(baseDeConnaissance * BDC) const
{
    for (unsigned int i(0); i<BDC->getRegles().size(); i++) //Parcours de toutes les règles
    {
        ...
        {
            if (this->getEltTest() == BDC->getRegles()[i].getResRegle()[j].getElement())    //Si même élément
            {
                if (this->getOperateur() == BDC->getRegles()[i].getResRegle()[j].getOperateur())    //Si même opérateur
                {
                    if (this->getValeurChaine() == BDC->getRegles()[i].getResRegle()[j].getValeur())    //Si même valeur
                    {
                        return true;
                    }
                }
